...
- **A part named in a panel's text is a `part-link`** (BLOCKS), usually through `prose-block` and a `prose` sentence (`model/prose.ts`): pointing lights the part on the grid and pressing switches to Edit and selects it. The block hands both gestures to `PART_LINK_TARGET`, which `main.ts` provides as `PartNavigationService`; a story or a spec provides its own. The gallery's Reuse backlog lists the places that still name a part their own way.
