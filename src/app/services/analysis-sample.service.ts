...
export class AnalysisSampleService {
  ...
  private forceSample(
    ...
    if (
      ...
      const valueSI =
        mechProp === 'Frictionless Input Effort'
          ? effort.valueSI - (frame.additionalFrictionEffort?.valueSI ?? 0)
          : effort.valueSI;
      return [
        (valueSI * (isForce ? forceConversion : torqueConversion)) / (isForce ? 1 : MODEL_SCALE),
      ];
    }
