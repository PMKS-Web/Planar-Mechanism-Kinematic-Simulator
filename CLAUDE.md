...
- A joint's **type** — Revolute, Prismatic, Pin-in-slot or Welded — is not stored. `model/joint-type.ts` reads it off the two facts the drawing records (a sliding block, a weld), says which of their edits a change of type runs and in what order, and composes those edits' refusals; `services/joint-type.service.ts` runs a change as one undo entry. The Edit panel and the group panel offer it as one choice (Stage 0 of `docs/joint-type-and-cylinder-plan.md`).
