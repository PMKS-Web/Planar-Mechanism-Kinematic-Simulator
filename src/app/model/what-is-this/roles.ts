...
/**
 * Each link's job, named the way a mechanisms course names it.
 *
...
export interface LinkJob {
  ...
  motion: string;
  /** The links it is about, for the panel to find their rows: a body, or a cylinder's two members. */
  links: Link[];
}

/**
 * The job as the panel's Links rows show it: its first words, without the
 * explanations the model is given ("bell crank (a rocker whose two arms meet
 * at an angle at its pivot)" is "Bell crank"; "cylinder; the block marked S on
 * it is…" is "Cylinder"; "input crank, carries traced point E" is "Input
 * crank"). A row has room for a name, not a sentence.
 */
export function panelRole(job: LinkJob): string {
  const words = job.job.split(/[;,](?![^(]*\))/)[0].replace(/\s*\(.*\)$/, '');
...
export function linkJobs(
  ...
  for (const body of ctx.bodies) {
    ...
    if (joints.every(isGroundPin)) {
      jobs.push({ name, job: 'part of the frame', motion: 'fixed to the ground', links: [body] });
      continue;
    }
    const pivot = joints.find(isGroundPin);
    ...
    if (pivot) {
      if (full) {
        ...
        const bell = beam ? undefined : bellCrankArms(ctx, body);
        job = isInput
      ...
      const coupled = !slotted && !onFixedGuide ? cranksCoupledBy(ctx, body) : [];
      job = slotted
  ...
  for (const cylinder of ctx.cylinders) {
    ...
    jobs.push({
      ...
      links: [cylinder.barrel, cylinder.rod],
    });
  }
  return jobs;
}
