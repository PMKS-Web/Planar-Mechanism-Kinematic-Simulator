...
try {
  for (const story of [
    ...
    await panel.waitFor();
    await panel.locator('.panel-content').waitFor();
    ...
    if (story === 'playback-disabled') {
      ...
      assert.equal(await panel.getByRole('button', { name: 'Apply Friction' }).isDisabled(), true);
    }
    ...
    await page.screenshot({ path: path.join(out, `${story}.png`) });
    results.push(story);
