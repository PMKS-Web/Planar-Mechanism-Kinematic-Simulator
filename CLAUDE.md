...
- **What Is This?** is the machine panel's plain-English note, written by Gemini from a fact sheet PMKS+ computes (`model/what-is-this/`) and a picture taken from the canvas (`services/what-is-this/`), through the Netlify Function `netlify/functions/what-is-this.mts`, which alone holds the key. The same sheet is never asked about twice: a note is filed under its sheet's key, kept in the browser, and the library's notes ship in `src/assets/what-is-this/library-notes.json`. The Links rows' jobs and the Family row come from the same sheet. `npm run what-is-this:dev` runs the function locally; [`docs/what-is-this-shipping.md`](docs/what-is-this-shipping.md) is the design, and `production-parity.spec.ts` holds the sheet to the one the evaluation measured.
