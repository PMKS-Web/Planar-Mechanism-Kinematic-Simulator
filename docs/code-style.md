...
`refuseJointOperation` in `model/joint-operation-permission.ts`, `refuseJointType` in
`model/joint-type.ts` for a change of joint type, and `locksHolding` in `model/lock-set.ts`. A
