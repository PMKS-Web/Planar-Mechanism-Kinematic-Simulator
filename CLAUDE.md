...
- MATLAB Analysis Package exports a plain engineering IR (`model/analysis-export.ts`) through `services/export/matlab-model.ts` and modular generators in `services/export/matlab/`. MATLAB independently solves supported kinematics and static/dynamic force equilibrium; PMKS reference data is optional. Keep the engineering explanation on the executable path: `equation-plan.ts` supplies named constraint rows, coordinate/force mappings and the guide; generated equation functions assemble the actual matrices, with a generic-model runtime check. Named results and grouped plots make the arrays readable; kinematics-only packages omit force files. `validate_pmks_package.m` reruns the actual generated solver and reports equation residuals, completion, conditioning and optional PMKS comparison. Both comparison export options default to No; the writer supplies a frame-zero SVG through the existing renderer. **Feature frozen pending actual MATLAB runtime testing: preserve this architecture and add no capabilities until those results are reviewed.** The old `.m` format is explicitly named PMKS Reference Script. `component/measurement-comparison/` compares pasted timestamped data through `model/measurement-comparison.ts`; measurements are panel-session state, not URL state. See `docs/matlab-and-measurements.md` for support, units, validation and the known PMKS dynamic-force display-scale discrepancy.
