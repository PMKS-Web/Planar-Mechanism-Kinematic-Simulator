...
- `cylinder-mount.mjs` — a cylinder mount as an ordinary attachment point: the toggles its menu now offers and the reason under the one it still grays, the live ring under a dragged joint (offered, refused with its reason, and the deliberate no-ring when a ram is dragged onto its own mount), a slot cut at a mount and the ram's sealed inside refusing one, a welded body selected as one part with the ram still its own beside it, one undo entry each way across the weld, a second machine that must not move while the first is edited, an unweld committed at a paused pose and re-anchored, and filmstrips of a compound drag and of a ram driving a block through its welded bracket
...
- `right-drawer.mjs` — the right drawer against the view controls: every page as wide as that card and on its left edge, the bottom one gap above it with the tutorial pinned and on a short window, both cards scrolling inside themselves rather than the frame, and the export note unclipped
- `link-holds.mjs` — holding a bar's length or angle: the Fixed Length / Fixed Angle rows carrying the value they would hold, the chip on the bar, the field's padlock reading Locked while the number stays typeable, the hairline hover dimension that leaves the chip as its label, a joint riding the arc about the far end with the amber guide drawn, the hold moving from length to angle with the message that offers the way back, a joint two holds leave nowhere to go refusing with the red ring and a Release button, the joint menu's grayed Free to Move row, undo taking the hold off (it rides the URL), and the chips standing down outside Edit
- `link-holds-angles.mjs` — a locked bar's name, its length chip and its center-of-mass mark at eight angles of the bar: the name and the chip part along the bar, and none of the three overlaps another. Writes a contact sheet under `artifacts/link-holds-angles/`, which is the check to look at
