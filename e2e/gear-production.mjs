...
try {
  ...
  const longName = 'Output shaft '.repeat(15).trim();
  await fill('gear-name', longName);
  await fill('gear-teeth', 10000);
  await fill('gear-diameter', 4);
  const dense = await look();
  assert.equal(dense.gears[1].name, longName);
  assert(dense.solved[0].diagnostics.some((d) => d.code === 'incompatible-module'));
  assert((await output.locator('.teeth').getAttribute('d')).match(/M/g).length <= 96);
  assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false);
  await film.shot('large-teeth-invalid-module-long-name');
  await fill('gear-teeth', 40);
  await fill('gear-diameter', 4);
  await fill('gear-name', 'Gear 2');
  assert.equal((await look()).solved[0].valid, true);
