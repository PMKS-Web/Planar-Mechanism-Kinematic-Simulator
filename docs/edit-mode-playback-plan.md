...
| On a swing wider than a turn, a drag of a few pixels flipped the ghost onto the other assembly branch. A provisional cycle's angles are unwrapped from wherever the hand is, so the stored start angle coincided first with the crossing a whole turn away -- the other branch -- and the true start, a turn further along, was never considered | Anchor lookup | `reachAnchor` gathers the crossings of every whole turn of the angle inside the cycle, picks the one nearest the seed, and refuses a winner that stands more than a fifth of the drawing from the seed -- the other branch wearing the start's angle -- so the ghost warns and the start moves rather than the design flipping under it. The seed is re-taken from the new start on every successful re-anchor (`reseedAnchor`) |
