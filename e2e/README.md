...
- `matlab-package.mjs` — actual app export of an independent MATLAB analysis ZIP, readable equations/named results, runtime-validator wiring, initial mechanism SVG, explicit SI wording, both optional comparison defaults and opt-ins, phone layout, unsupported-driver refusal and kinematics-only omission of force files. Uses configured four-bar masses/inertias; artifacts include the extracted package and screenshots. These checks do not execute MATLAB. Set `PMKS_BASE_URL` to the feature worktree server for both this suite and `matlab-measurements.mjs`.
